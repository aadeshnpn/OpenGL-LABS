...
glLoadIdentity();
gluPerspective(45,float(width)/float(height),1,450); //Default for solid GLU objects
...
glColor3f(1.0, 1.0, 0.0); // makes the current color red
glPushMatrix(); // pushes the current matrix stack down and makes a copy on top
glTranslatef(-5, 20, -100); // multiplies the top of stack with trans matrix
glutWireCone(20, 30, 20, 20); // creates a wire cone using current matrix
